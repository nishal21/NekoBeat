...
fn spotify_cookie() -> Result<String, String> {
    ...
    Err(
        "Set NEKOBEAT_SPOTIFY_COOKIE env var with your sp_dc cookie for Spotify synced lyrics"
            .into(),
    )
}
...
async fn get_latest_secret_key_version(client: &Client) -> Result<(Vec<u8>, String), String> {
    ...
    let map = secrets_data
        .as_object()
        .ok_or("Invalid secrets JSON format")?;
...
async fn get_server_time_params(client: &Client) -> Result<(String, String, String), String> {
    ...
    let server_time_seconds = data["serverTime"]
        .as_u64()
        .ok_or("Invalid server timestamp")?;
...
async fn get_token(client: &Client) -> Result<String, String> {
    ...
    let access_token = token_json["accessToken"]
        ...
        .to_string();
    let expiration = token_json["accessTokenExpirationTimestampMs"]
        .as_u64()
        .ok_or("No expiration found")?;
...
pub async fn get_spotify_lyrics(track_id: String) -> Result<MusixmatchResponse, String> {
    ...
    match get_spotify_lyrics_impl(&track_id).await {
        Ok(lyrics) => {
            println!(
                "Successfully fetched Spotify lyrics for track_id: {}",
                track_id
            );
            Ok(lyrics)
        }
        Err(e) => {
            eprintln!(
                "Failed to fetch Spotify lyrics for track_id: {}: {}",
                track_id, e
            );
            Err(e)
        }
...
async fn get_spotify_lyrics_impl(track_id: &str) -> Result<MusixmatchResponse, String> {
    ...
    let data: Value = serde_json::from_str(&text)
        .map_err(|e| format!("Failed to parse lyrics json: {}, body: {}", e, text))?;

    let lyrics_lines = data["lyrics"]["lines"]
        .as_array()
        .ok_or("Invalid lyrics format")?;
...
mod tests {
    ...
    async fn test_get_spotify_lyrics() {
        ...
        match result {
            Ok(r) => println!(
                "Success! Lyrics length: {}",
                r.synced_lyrics.unwrap_or_default().len()
            ),
