...
async fn generate_signature(secret: &str, url: &str) -> Result<String, String> {
    ...
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes()).map_err(|_| "HMAC Error")?;
...
pub async fn get_musixmatch_lyrics(
    title: String,
    artist: String,
) -> Result<MusixmatchResponse, String> {
    ...
    println!(
        "Musixmatch: Found Track ID {} for '{} - {}'",
        track_id, title, artist
    );

    // 2. Fetch Subtitles! (Synced Lyrics)
    let sub_endpoint = format!(
        "track.subtitle.get?app_id=community-app-v1.0&format=json&track_id={}",
        track_id
    );
        ...
        if let Some(sub_body) = res["message"]["body"]["subtitle"]["subtitle_body"].as_str() {
            ...
            if let Ok(parsed_subs) = serde_json::from_str::<Value>(sub_body) {
                if let Some(arr) = parsed_subs.as_array() {
                    ...
                    for item in arr {
                        if let (Some(text), Some(time_tot)) =
                            (item["text"].as_str(), item["time"]["total"].as_f64())
                        {
    ...
    let lyr_endpoint = format!(
        "track.lyrics.get?app_id=community-app-v1.0&format=json&track_id={}",
        track_id
    );
    if let Ok(res) = make_request(&client, &lyr_endpoint).await {
        if let Some(lyr_body) = res["message"]["body"]["lyrics"]["lyrics_body"].as_str() {
            ...
            if let Some(idx) =
                clean_lyrics.find("******* This Lyrics is NOT for Commercial use *******")
            {
